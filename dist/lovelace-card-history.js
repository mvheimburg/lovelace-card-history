...
const LEFT = 44, TOP$1 = 24, PLOT_BOTTOM = 196, 
...
function lineChart(all, start, end, hover, text, options = {}) {
    ...
    ${xTicks.map((t) => w `<line class="grid" x1=${x(t)} x2=${x(t)} y1=${TOP$1} y2=${END}></line><text class="axis" x=${x(t)} y=${END + 18} text-anchor="middle">${text.time(t, every >= 24)}</text>`)}
...
function timeline(lanes, start, end, hover, text, W = 600) {
    ...
    ${lanes.map((lane, i) => {
        const y = top(i) + LABEL;
        return w `<g class="band-lane" data-lane=${lane.kind}>
...
class HistoryCard extends i {
    ...
    setConfig(config) {
        ...
        const hours = RANGES.includes(config.hours) ? config.hours : 24;
        this.config = {
            fill: true,
            smooth: false,
            show_current: true,
