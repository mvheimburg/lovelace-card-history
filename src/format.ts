...
export function historyFormat(hass?: LanguageSource) {
  ...
  return {
    ...
    reading: (value: number, unit = "", digits = 1) =>
      `${safe(() => new Intl.NumberFormat(locale, { maximumFractionDigits: digits }).format(value), String(value))}${unit ? ` ${unit}` : ""}`,
